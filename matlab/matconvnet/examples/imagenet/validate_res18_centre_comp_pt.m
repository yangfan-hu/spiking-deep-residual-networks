...
load([opts.expDir 'resnet18-pt-mcn-activations']); %load sampled activation data
...
for l = 1:numel(net.layers)
   if isa(net.layers(l).block,'dagnn.Conv')
       if strcmp(net.layers(l).name,'classifier_0')
           ...
           %conv layer
           if l == 5 %res18 has no adapter in first res block
           ...
           if isa(net.layers(l + 2).block, 'dagnn.ReLU')  %regular layer   %same 
               lambda = max_activation(net.getVarIndex(net.layers(l + 2).outputs));
           elseif isa(net.layers(l + 3).block, 'dagnn.ReLU') %2nd conv before sum or adapter before sum
                lambda = max_activation(net.getVarIndex(net.layers(l + 3).outputs));
           elseif isa(net.layers(l + 5).block, 'dagnn.ReLU') %2nd conv before adapter 
...
for l = 1:numel(net.layers)
    if isa(net.layers(l).block, 'dagnn.Sum')
        ...
        if l == 10  %first block
            ...
            adapter(i).lambda_previous = max_activation(net.getVarIndex(net.layers(l - 7).outputs));   %resblock of 2        
            i = i + 1;
        elseif isa(net.layers(inputindex).block, 'dagnn.ReLU') 
            adapter(i).tarname = net.layers(inputindex).name;
            adapter(i).sumname = net.layers(l).name;     
            adapter(i).lambda = max_activation(net.getVarIndex(net.layers(l + 1).outputs));
            adapter(i).lambda_previous = max_activation(net.getVarIndex(net.layers(l - 6).outputs));   %resblock of 2
            i = i + 1;
        end
    end
