...
load([mainDir '/net-epoch-113.mat']); 
...
load([mainDir '/res-' num2str(depth) '-activations.mat']); %load sampled activation data
...
testdata = images.data(:,:,:,images.set == 3);  %important select centre 
truth = images.labels(images.set==3);
result = zeros(size(truth));
